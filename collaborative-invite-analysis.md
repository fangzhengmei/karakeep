...
#### 6. 协作者被移除后的数据处理（原说法修正 + 测试证据链）

> **原说法不准确修正：** 原描述"协作者被移除后其添加的书签仍会保留在列表中"不符合实际代码行为。

**真实行为结论（基于代码 + 数据库级联约束）：**
1. ✅ **列表中只剩余所有者书签**：协作者添加的书签关联会被级联删除
2. ✅ **协作者书签仍保留在其个人库**：书签是独立实体，只删除列表关联，不删除书签本身
...
3. **移除协作者时**（`lists.ts:715-722`）：
   ...
   - **不影响** `bookmarks` 表中的书签记录本身

4. **书签是独立实体**（`schema.ts:188-222`）：
   - `bookmarks` 表通过 `userId` 字段直接关联到创建者
   - 不依赖任何列表关联而存在
   - 删除 `bookmarksInLists` 记录不会级联删除 `bookmarks` 记录

**测试证据链（基于现有测试代码逻辑推导）：**

| 测试场景 | 验证点 | 支撑结论 |
|---------|--------|----------|
| `add and remove from list` (`lists.test.ts:303-320`) | 书签可添加到列表并移除，书签本身仍然存在 | 证明 `bookmarksInLists` 是关联表，删除关联不影响 `bookmarks` 表 |
| `merge lists` (`lists.test.ts:91-129`) | 合并列表后，书签从源列表移动到目标列表，书签 ID 不变 | 证明书签是独立实体，列表只是组织方式 |
| `getListsOfBookmark` (`lists.test.ts:124-126`) | 可查询书签所在的所有列表 | 证明书签与列表是多对多关系，书签独立存在 |
| `leaveList` 注释 (`lists.ts:735`) | "This also removes all bookmarks that the user added to the list." | 代码注释明确说明移除协作者时会删除其添加的书签关联 |

**页面表现：**
- 协作者被移除后，列表中该协作者添加的所有书签都会消失
- 列表所有者添加的书签不受影响，继续保留
- 被移除的协作者登录后，在自己的书签库中仍能看到这些书签

**用户感知差异：**
- **用户可能期望：** "移除协作者只是不让他们继续编辑，之前添加的内容应该保留在列表中"
- **实际行为：** 协作者添加的所有书签都会从列表中移除（但仍在他们自己的库中）
- **潜在困惑：** 列表所有者可能疑惑"那些书签去哪儿了？"
- **建议改进：** 移除协作者时可提供选项"是否保留该协作者添加的书签？"
