...
用户上传图片/PDF
  ...
  ├─ OpenAIQueue（tag）
  ├─ OpenAIQueue（summarize）  ← 所有类型都会入队，但非 LINK 在推理阶段被跳过
  └─ SearchIndexingQueue
      │
      ▼
EmbeddingsWorker 处理：
...
5. **摘要仅限 LINK 类型**：资产预处理为所有类型入队摘要任务，但推理阶段对 IMAGE/PDF 直接跳过（`return`），不写回摘要，不触发搜索索引；`summarizationStatus` 被标记为 `"success"` 但 `summary` 保持 `null`

### 12.2 任务调度与重试

6. **任务解耦**：每个处理阶段都是独立的队列任务，可独立扩展和重试
7. **幂等性设计**：使用 idempotencyKey 避免重复处理
8. **分级重试**：不同队列有不同的重试次数（2-5次）和策略
9. **指数退避 + 全抖动**：重试延迟采用指数增长 + 随机化，避免惊群效应
10. **速率限制特殊处理**：`QueueRetryAfterError` 不消耗重试次数

### 12.3 下游任务触发

11. **非 fixMode 总是触发**：默认模式下，无论 OCR 是否成功，都会触发下游任务（下游任务自行处理无内容场景）
12. **fixMode 增量触发**：修复模式下，只有内容实际变化时才触发下游任务，避免无效计算

### 12.4 可靠性与公平性

13. **优雅降级**：嵌入失败时仍能进行 tagging（不带相似度上下文）
14. **按用户分组**：任务调度时按用户分组，确保资源公平分配
15. **信号量控制**：Restate Dispatcher 使用信号量控制并发，避免系统过载
16. **批量策略**：首次运行启用批量处理以提高性能，重试时禁用批量以提高可靠性
