...
用户上传图片/PDF
  ...
  AssetPreprocessingWorker 处理
      ...
      │
      ├─ 图片类型：
      │    ├─ LLM OCR（如果启用）→ 失败回退到 Tesseract
      │    └─ 提取文本 → 写入 bookmarkAssets.content
      │
      └─ PDF 类型：
           ├─ 提取文本 + metadata → 写入 bookmarkAssets
...
1. **双引擎 OCR**：结合传统 OCR（Tesseract）和 LLM OCR，兼顾成本和准确率
2. **任务解耦**：每个处理阶段都是独立的队列任务，可独立扩展和重试
3. **幂等性设计**：使用 idempotencyKey 避免重复处理
4. **分级重试**：不同队列有不同的重试次数和策略
5. **优雅降级**：嵌入失败时仍能进行 tagging（不带相似度上下文）
6. **Fix Mode**：支持增量修复，避免重复处理已有内容
7. **按用户分组**：任务调度时按用户分组，确保资源公平分配
8. **信号量控制**：Restate Dispatcher 使用信号量控制并发，避免系统过载
